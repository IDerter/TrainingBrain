...
public class OnClickButton3 : MonoBehaviour
{
    ...
    void Start()
    {
       ...
       // Camera.main.aspect = 1080f / 1920f;
        ...
        StartCoroutine(WaitAndPrint7(8f));
        NewMasiveRandom[b] = x;
        b += 1;
        StartCoroutine(WaitAndPrint8(11f));
        /*if (k == 0)
        {
            j = 1;
        }
    ...
    void Update()
    {
        ...
        if (PlayerTimer == true && timeintwork >= 11f)
        {
        ...
        if ((NewMasiveRandom[0] == NewMasiveInput[0]) && (NewMasiveRandom[1] == NewMasiveInput[1]) && (NewMasiveRandom[2] == NewMasiveInput[2]) && (NewMasiveRandom[3] == NewMasiveInput[3]) && (NewMasiveRandom[4] == NewMasiveInput[4] && (NewMasiveRandom[5] == NewMasiveInput[5] && (NewMasiveRandom[6] == NewMasiveInput[6]) && (NewMasiveRandom[7] == NewMasiveInput[7])&&(NewMasiveRandom[8]==NewMasiveInput[8]))))
        {
            ...
            if (PlayerPrefs.GetInt("Score3") > timeint && PlayerPrefs.HasKey("Score3") == true)
            {
                timeint = (int)timeStart;
                PlayerPrefs.SetInt("Score3", timeint);
            ...
            {
                if (PlayerPrefs.GetInt("Score3") == 0 && PlayerPrefs.HasKey("Score3") == false)//если игрок с первой попытки нажал все правильно
                {
        ...
        {
            if ((a == 9) && (PlayerPrefs.HasKey("Score3") == false))
            {
                //timeStart = 0f;
    ...
    public void OnClickEnterBlack()
    {
        ...
        if (myButton[8].name == "ButtonBlack")
        {
            a++;
            NewMasiveInput[d] = 8;
            d++;
