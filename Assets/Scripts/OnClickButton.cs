...
public class OnClickButton: MonoBehaviour {
   ...
   // public GameManager script;
    ...
    void Update() {
        ...
        if (PlayerTimer == true && timeintwork >= 10f)
        {
        ...
        if ((NewMasiveRandom[0]==NewMasiveInput[0])&& (NewMasiveRandom[1] == NewMasiveInput[1]) && (NewMasiveRandom[2] == NewMasiveInput[2]) && (NewMasiveRandom[3] == NewMasiveInput[3]) && (NewMasiveRandom[4] == NewMasiveInput[4] && (NewMasiveRandom[5] == NewMasiveInput[5] && (NewMasiveRandom[6] == NewMasiveInput[6]))))
        {
           // timeStart = 1000f;
            ...
            if(PlayerPrefs.GetInt("Score")>timeint&& PlayerPrefs.HasKey("Score") == true)
            {
                timeint=(int)timeStart;
           ...
           // {
             ...
             //   }
                ...
                {
                    if (PlayerPrefs.GetInt("Score")==0 && PlayerPrefs.HasKey("Score") == false)//если игрок с первой попытки нажал все правильно
                    {
        ...
        {
            if ((a == 7)&& (PlayerPrefs.HasKey("Score") == false))
            {
                //timeStart = 0f;
    ...
    public void OnClickEnterPurple()
    {
        ...
        if (myButton[5].name == "ButtonPurple")
        {
            a++;
            NewMasiveInput[d] = 5;
            d++;
